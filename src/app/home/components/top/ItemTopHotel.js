...
const ItemTopHotel = (props) => {
  const minPrice = props.hotel.rooms.reduce((result, room) => {
    return result > room.price ? room.price : result; 
  }, 10000);
  ...
  const rating = (rate) => {
    ...
    for(let i=0; i<5; i++){
      ...
      else if(num == 0.5) arrRating.push(
        <li key={i}>
  ...
  return (
    <NavLink to = {`/hotels/${transformUrl(props.hotel.name, props.hotel._id)}`}>
      <div className="item-service grid-item has-matchHeight">
        <div className="service-border">
          <div className="thumb">
            ...
            >
              <div
                className="service-add-wishlist"
            ...
            <a>
              <img
                width={680}
                height={500}
                src={`${config.Url}/${props.hotel.images[0]}`}
          ...
          <p className="service-location plr15">
            ...
            {`${props.hotel.city.name}, ${props.hotel.city.country}`}
          </p>
          ...
          <div className="service-review plr15">
            <ul className="icon-group text-color booking-item-rating-stars">
              {
                rating(props.hotel.rate)
              }
            </ul>
            <span className="review">{`${props.hotel.num_review} Reviews`}</span>
          </div>
            ...
            <div className="footer-inner plr15">
              <div className="service-price">
                ...
                <span className="price">
                  <span className="text-lg lh1em item ">€{minPrice}/night</span>{" "}
                </span>
              </div>
            </div>
          </div>
