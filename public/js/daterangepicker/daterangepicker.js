                ...
                if (!this.showCalendar) {
                    if (this.sameDate) {
                        if($(e.target).parent().hasClass('start-group')){
                            var endGroup = $(e.target).parent().nextAll('.end-group').first();
                            var end_title = endGroup.attr('data-title');
                            var end_row   = end_title.substr(1, 1);
                            var end_col   = end_title.substr(3, 1);
                            var end_date  = cal.hasClass('left') ? this.leftCalendar.calendar[end_row][end_col] : this.rightCalendar.calendar[end_row][endcol];
